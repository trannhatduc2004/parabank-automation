...
- Name: e2e\tests\account.spec.ts >> Account Overview >> có ít nhất 1 tài khoản @regression
- Location: src\e2e\tests\account.spec.ts:22:7
    ...
    - generic [ref=e27]:
      ...
      - generic [ref=e50]:
        ...
        - table [ref=e52]:
          ...
          - rowgroup [ref=e58]:
            - row "12345 -$500.00 $0.00" [ref=e59]:
              - cell "12345" [ref=e60]:
                - link "12345" [ref=e61] [cursor=pointer]:
                  - /url: activity.htm?id=12345
              - cell "-$500.00" [ref=e62]
              - cell "$0.00" [ref=e63]
            - row "12456 $210.45 $210.45" [ref=e64]:
              - cell "12456" [ref=e65]:
            ...
            - row "Total $3692.67" [ref=e114]:
              - cell "Total" [ref=e115]
              - cell "$3692.67" [ref=e116]
              - cell [ref=e117]
          - rowgroup [ref=e118]:
...
```ts
  ...
  4  | 
  5  | test.describe('Account Overview', () => {
  6  | 
  7  |   test.beforeEach(async ({ page }) => {
  8  |     const loginPage = new LoginPage(page);
  9  |     await loginPage.goto();
  10 |     await loginPage.login(
  11 |       process.env.TEST_USER!,
  12 |       process.env.TEST_PASSWORD!
  13 |     );
  14 |     await loginPage.expectLoginSuccess();
  15 |   });
  16 | 
  17 |   test('hiển thị danh sách tài khoản sau khi login @smoke', async ({ page }) => {
  18 |     const overviewPage = new AccountOverviewPage(page);
  19 |     await overviewPage.expectAccountTableVisible();
  20 |   });
  21 | 
  22 |   test('có ít nhất 1 tài khoản @regression', async ({ page }) => {
  23 |     const overviewPage = new AccountOverviewPage(page);
  24 |     await overviewPage.expectAccountTableVisible();
  25 |     const count = await overviewPage.getAccountCount();
> 26 |     expect(count).toBeGreaterThanOrEqual(1);
     |                   ^ Error: expect(received).toBeGreaterThanOrEqual(expected)
  27 |   });
  28 | 
  29 |   test('hiển thị welcome message đúng @regression', async ({ page }) => {
  30 |     const overviewPage = new AccountOverviewPage(page);
  31 |     await overviewPage.expectAccountTableVisible();
  32 |     // Check tên user hiển thị ở đâu đó trên trang
  33 |     await expect(page.locator('text=John')).toBeVisible({ timeout: 5000 });
  34 |     });
  35 | 
  36 | });
```